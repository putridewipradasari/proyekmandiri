...
class Event extends CI_Controller
{
    ...
    public function list_admin()
    {
        ...
        $this->template->_render_page('backend', $this->data);
        //print_r($this->data);
        //echo json_encode($this->data['_get_event']);
    ...
    public function tambah_aksi()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!$this->upload->do_upload('berkas')) {
                ...
                $sql = $this->db->query("SELECT email FROM `users` JOIN `profil` ON users.id = profil.id_user JOIN `users_groups` ON profil.id_user = users_groups.user_id WHERE ekskul = '$ekskul' and group_id = '2'");
                //$this->db->query($sql);
                //print_r ($sql);
                //echo $sql;
                ...
                $this->session->set_flashdata('message', 'Data berhasil ditambahkan dan dikirim');
                redirect(site_url('event/list_admin'));
            }
        }
    }
